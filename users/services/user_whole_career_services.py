...
from users.serializers import UserProfileSerializer, UserCareerExperienceSerializer, UserEducationSerializer
...
class UserWholeCareerServices:
    ...
    def get_user_whole_career(self):
        ...
        user_carrier_experiences = UserCareerExperience.objects.filter(user=user, deleted_at__isnull=True)
        user_educations = UserEducation.objects.filter(user=user, deleted_at__isnull=True)

        user_profile_serializer = UserProfileSerializer(user_profile)
        user_carrier_serializer = UserCareerExperienceSerializer(user_carrier_experiences, many=True)
    ...
    def update_user_whole_career(self, request_data):
        ...
        user_profile_serializer = UserProfileSerializer(user_profile, data=request_data.get('profile'))
        ...
        user_carrier_serializer = UserCareerExperienceSerializer(data=request_data.get('career_experiences'), many=True)
        user_carrier_serializer.is_valid(raise_exception=True)
        user_carrier_serializer.save(user=user)

        user_education_serializer = UserEducationSerializer(data=request_data.get('educations'), many=True)
